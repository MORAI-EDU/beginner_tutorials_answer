...
# mgeo_dijkstra_path_2 은 Mgeo 데이터를 이용하여 시작 Node 와 목적지 Node 를 지정하여 Dijkstra 알고리즘을 적용하는 예제 입니다.
# 사용자가 직접 지정한 시작 위치 와 목적지 위치 사이 최단 경로 계산하여 global Path(전역경로) 를 생성 합니다.

# 노드 실행 순서 
# 1. Mgeo data 읽어온 후 데이터 확인
# 2. 시작 위치 와 종료 위치 subscribe 후 가장 가까운 Node 탐색
# 3. weight 값 계산
...
class dijkstra_path_pub(Node):
    def __init__(self):
        super().__init__('dijkstra_path_pub')
    ...
    def timer_callback(self):
        if self.is_goal_pose and self.is_init_pose:
            ...
            self.get_logger().info('Waiting goal pose data', throttle_duration_sec=1.0)
            self.get_logger().info('Waiting init pose data', throttle_duration_sec=1.0)
...
class Dijkstra:
    ...
    def find_shortest_path(self, start_node_idx, end_node_idx): 
        ...
        for i in range(len(node_path) - 1):
            ...
            from_node_obj = self.nodes[from_node_idx]
            to_node_obj = self.nodes[to_node_idx]

            shortest_link, min_cost = self.find_shortest_link_leading_to_node(from_node_obj, to_node_obj)
...
def main(args=None):
    ...
    dijkstra_path_publisher = dijkstra_path_pub()
    
    try:
        rclpy.spin(dijkstra_path_publisher)
    except KeyboardInterrupt:
        pass
    finally:
        dijkstra_path_publisher.destroy_node()
        rclpy.shutdown()
